...
- **Identity:** Artificial Superintelligence is the primary heading, ASI Atlas the secondary identity. The prominent **IT IS HERE ALREADY!** badge opens the creator’s personal prediction as an AI student. This replaces the earlier arrival window without changing the evidence classification of any capability. Text and provenance are editable in `src/content/arrival.ts`.
