...
if (
  originalAtlas.length !== 68 ||
  new Set(originalAtlas.map((r) => r.id)).size !== 68 ||
  originalAtlas.some((r, i) => Number(r.id) !== i + 1)
)
  throw new Error('Original Atlas must contain IDs 1–68 exactly once.');
const extensions = JSON.parse(
  await readFile(path.join(content, 'documents/ATLAS_EXTENSIONS.json'), 'utf8'),
);
if (
  extensions.length !== 4 ||
  extensions.some((r, i) => Number(r.id) !== 69 + i || r.provenance !== 'O→F')
)
  throw new Error('Later extensions must contain T69–T72 exactly once with O→F provenance.');
const atlas = [
  ...originalAtlas.map((r) => ({ ...r, corpus: 'Original conversation corpus' })),
  ...extensions,
];
const corpusNote =
  '<p><strong>Original conversation corpus:</strong> 68 theses/proposals (T1–T68). <strong>Later research extensions:</strong> T69–T72. <strong>Current Atlas:</strong> 72 entries.</p>';
...
function cards() {
  return `<div class="paper-cards">${papers.map((d) => `<article class="paper-card"><div class="paper-marker">${d.id}<span>v0.1</span></div><div><p class="status-badge">${e(d.status)}</p><h2><a href="${d.route}">${e(d.id === 'P8' ? 'The Embodiment Threshold' : d.title)}</a></h2>${d.id === 'P8' ? '<p>Persistent Controllers, Re-Embodiment, and Actuation Overhang in Advanced AI</p>' : ''}<p class="card-meta">${author} · Manuscript <time datetime="${d.date}">${d.date === '2026-09-15' ? '15' : '14'} September 2026</time></p><p>${e(d.cardDescription)}</p><div class="card-links"><a class="read-paper" href="${d.route}">Read paper <span aria-hidden="true">↗</span></a><a href="${d.sourceUrl}">View source</a></div></div></article>`).join('')}</div>`;
}
...
function destination(value) {
  return e(value)
    .replace(/\bP[1-8]\b/g, (id) => `<a href="${papers.find((p) => p.id === id).route}">${id}</a>`)
    .replace(/\bRN[1-4]\b/g, (id) => `<a href="/research/atlas?destination=${id}">${id}</a>`);
}
const atlasRows = atlas
  .map(
    (r) =>
      `<tr id="T${r.id}" data-thesis data-id="${r.id}" data-corpus="${e(r.corpus)}" data-provenance="${e(r.provenance)}" data-epistemic_status="${e(r.epistemic_status)}" data-primary_destination="${e(r.primary_destination)}"><th scope="row"><a href="#T${r.id}">T${r.id}</a></th><td><h2>${e(r.title)}</h2><div class="atlas-tags"><span>${e(r.provenance)}</span><span>${e(r.epistemic_status)}</span><span>${e(r.corpus)}</span></div>${r.core_claim ? render(r.core_claim).html : ''}${r.notes ? `<p>${e(r.notes)}${r.id === '72' ? ' <a href="/research/papers/embodiment-threshold#8-embodiment-only-delay-bound-theorem">Read the assumptions and proof.</a>' : ''}</p>` : ''}${r.id === '15' ? `<p class="historical-qualification">Historical conjecture · Not established. <a href="${papers[3].route}#${papers[3].toc.find((h) => h.text.startsWith('6. Why')).id}">Read P4’s counterexamples.</a></p>` : ''}<details><summary>Assumptions, falsifiers & prior art</summary><dl>${[
...
await page(
  ...
  '72 Atlas entries preserving provenance, epistemic status, assumptions, falsifiers and paper destinations.',
  `${titleBlock('Superintelligence Research Atlas', 'Research / 72 Atlas entries', 'The Atlas preserves 68 original research theses and proposals alongside four clearly marked later extensions. It records provenance, epistemic status, assumptions, falsifiers, prior art and the papers into which stronger ideas were consolidated.')}<div class="atlas-intro">${corpusNote}<p>v0.2 · Original corpus dated 14 September 2026 · Later extension and HTML publication dated 15 September 2026</p><p>Historical formulations are preserved rather than silently rewritten. Labels describe the source’s classification: “REPLICATION” here is not evidence that a replication experiment was run. T14 remains CONJECTURE despite “Theorem” in its title. RN1–RN4 name research-note groups, not supplied standalone manuscripts.</p><details class="provenance-key"><summary>Provenance key & epistemic status</summary><dl>${Object.entries(
    provenance,
  )
    .map(([key, text]) => `<div><dt>${e(key)}</dt><dd>${e(text)}</dd></div>`)
    .join(
      '',
    )}</dl><p>Statuses in this dataset: ${[...new Set(atlas.map((r) => r.epistemic_status))].map(e).join(' · ')}. T72 is a THEOREM only under its stated mathematical assumptions, with the proof in P8; it establishes no empirical pathway dates. No new empirical result is reported.</p></details><p><a href="/research/sources/SUPERINTELLIGENCE_RESEARCH_ATLAS.csv">Download original 68-row CSV</a> · <a href="/research/sources/ATLAS_EXTENSIONS.json">Download T69–T72 extension</a> · <a href="/research/sources/ATLAS_CURRENT.csv">Download current 72-entry CSV</a> · <a href="/research/clustering">Clustering matrix</a></p></div><form id="atlas-filters" class="atlas-filters" hidden><label class="atlas-search">Search all fields<input type="search" name="search" placeholder="e.g. control, evidence, T16" autocomplete="off"></label>${keys.map(([key, label]) => `<label>${label}<select name="${key}" aria-label="${e(label)}">${option('', 'All')}${[...new Set(atlas.map((r) => r[key]))].map((v) => option(v, key === 'id' ? `T${v}` : v)).join('')}</select></label>`).join('')}<button type="reset">Reset filters</button></form><p id="atlas-count" role="status">72 of 72 thesis records</p><noscript><p>All 72 records are available below. Enable JavaScript to use search and filters.</p></noscript><div class="atlas-table-wrap"><table class="atlas-table"><caption class="visually-hidden">The 72-entry research Atlas; open a record’s details for assumptions, falsifiers and prior art.</caption><thead><tr><th scope="col">ID</th><th scope="col">Thesis & evidence status</th><th scope="col">Paper connections</th></tr></thead><tbody>${atlasRows}</tbody></table></div><p id="atlas-empty" hidden>No records match these filters. Reset them or try a broader search.</p>${authorship}`,
  { version: 'v0.2', date: '2026-09-15' },
);
...
await cp(path.join(content, 'versions'), path.join(output, 'sources/versions'), {
  recursive: true,
});
const csvKeys = [...new Set(atlas.flatMap((r) => Object.keys(r)))];
const csvCell = (v = '') => '"' + String(v).replaceAll('"', '""') + '"';
await writeFile(
  path.join(output, 'sources/ATLAS_CURRENT.csv'),
  [csvKeys, ...atlas.map((r) => csvKeys.map((key) => r[key] ?? ''))]
